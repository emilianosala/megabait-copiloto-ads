...
import { buildJairSystemPrompt } from '@/lib/jair';
...
export async function POST(request: Request) {
  try {
  const { clientId, message, history, model, images, image } = await request.json();

  // Normalizamos: el frontend nuevo manda `images` (array). Mantenemos
  // compatibilidad con `image` (singular) por si queda algún request viejo.
  const currentImages: { mediaType: string; data: string }[] = Array.isArray(images)
    ? images
    : image
      ? [image]
      : [];
  ...
  // El cerebro de Jair vive en lib/jair (arquitectura de dos niveles: nucleo
  // siempre presente + modulos tematicos bajo demanda). Aca solo armamos el
  // contexto y delegamos el ensamblado del system prompt.
  const systemMessage = buildJairSystemPrompt({
    client,
    hoyLargo,
    hoyISO,
    hasMetaAds,
    hasGoogleAds,
    hasGA4,
    hasGoogleConnection: !!googleRefreshToken,
    hasSalesData,
    activeAlerts,
  });
  ...
  type IncomingMsg = {
    role: 'user' | 'assistant';
    content: string;
    // El frontend nuevo manda `images` (array). `image` (singular) queda por
    // compatibilidad con historiales/requests viejos.
  ...
  const toParam = (m: IncomingMsg): Anthropic.MessageParam => {
    ...
    if (imgs.length > 0 && m.role === 'user') {
      const blocks: Anthropic.ContentBlockParam[] = imgs.map((im) => {
        const mt: AllowedImg = (ALLOWED_IMG as readonly string[]).includes(im.mediaType)
          ? (im.mediaType as AllowedImg)
          : 'image/jpeg';
        return { type: 'image', source: { type: 'base64', media_type: mt, data: im.data } };
      });
  ...
  const messages: Anthropic.MessageParam[] = [
    ...historyParams,
    toParam({ role: 'user', content: message, images: currentImages }),
  ];
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      const savedUserContent = n > 0
        ? (message?.trim()
            ? `📎 ${message}`
            : `📎 (${n} captura${n > 1 ? 's' : ''} adjunta${n > 1 ? 's' : ''})`)
        : message;
